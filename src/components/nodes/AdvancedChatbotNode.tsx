...
interface AdvancedNodeData {
  ...
  message?: string;
  options?: string[];
  onAddNode?: (type: string, position: { x: number; y: number }, sourceId: string) => void;
}

export const AdvancedChatbotNode = memo(({ data, id, positionAbsoluteX, positionAbsoluteY }: NodeProps<AdvancedNodeData>) => {
  ...
  const handleAddNode = (type: string) => {
    if (data.onAddNode && typeof positionAbsoluteX === 'number' && typeof positionAbsoluteY === 'number') {
      data.onAddNode(type, { x: positionAbsoluteX, y: positionAbsoluteY + 150 }, id);
    }
  ...
  return (
    <div className="group relative">
      {/* Node Content */}
      <div className={`
        relative min-w-[240px] rounded-xl shadow-elegant transition-all duration-300
      ...
      `}>
        ...
        <div className={`bg-gradient-to-r ${gradientColor} p-3 rounded-t-xl flex items-center gap-2`}>
          ...
          <span className="text-white font-semibold text-sm">{data.label || 'Nodo'}</span>
        </div>

        {/* Body */}
        <div className="p-4 space-y-2">
          <p className="text-sm text-foreground leading-relaxed">
            {data.message || 'Configura este nodo...'}
          </p>
          
          {Array.isArray(data.options) && data.options.length > 0 && (
            <div className="space-y-1 mt-3">
              {data.options.map((option: string, i: number) => (
                <div key={i} className="text-xs px-2 py-1 bg-muted rounded flex items-center gap-1">
      ...
      {/* Radial Menu Button - ESTA ES LA MAGIA DE LOVABLE */}
      {data.type !== 'end' && (
        <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 z-50">
          <button
            className="w-10 h-10 gradient-primary rounded-full shadow-glow hover:scale-110 transition-all duration-300 flex items-center justify-center group-hover:animate-pulse"
            onClick={(e) => {
              e.stopPropagation();
          ...
          {/* Radial Menu - INNOVACIÓN DE LOVABLE */}
          {showRadialMenu && (
              ...
              <div className="relative w-[280px] h-[280px]">
                {nodeTypeOptions.map((option, index) => {
                  ...
                  return (
                    <button
                      key={option.type}
                      className={`absolute top-1/2 left-1/2 w-12 h-12 bg-gradient-to-br ${option.color} rounded-full shadow-elegant hover:shadow-glow hover:scale-125 transition-all duration-300 flex items-center justify-center group/item`}
                      style={{
                    ...
                    >
                      ...
                      <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-xs font-medium whitespace-nowrap opacity-0 group-hover/item:opacity-100 transition-opacity bg-card px-2 py-1 rounded shadow-elegant">
                        {option.label}
                      </span>
                    </button>
                  );
...
AdvancedChatbotNode.displayName = 'AdvancedChatbotNode';
